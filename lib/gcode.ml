...
let create_state () = { current_pos = { x = 0.0; y = 0.0 }; pen_down = false; lines = [] }
...
let check_machine_bounds (ir : ir) : bool =
  if ir = [] then true
    ...
    b.min_x >= machine_bounds.min_x && b.max_x <= machine_bounds.max_x &&
    b.min_y >= machine_bounds.min_y && b.max_y <= machine_bounds.max_y
...
let fit_to_machine ?(margin = 10.0) (ir : ir) : ir =
  if ir = [] then ir
    ...
    transform_ir (fun v -> { x = (v.x *. scale) +. offset_x; y = (v.y *. scale) +. offset_y }) ir
...
let generate ?(config = default_config) (ir : ir) : string =
  ...
  List.iteri (fun i path ->
    if config.include_comments then emit state (Printf.sprintf "; Path %d" (i + 1));
...
let count_segments (paths : path list) : int * int =
  List.fold_left (fun (moves, lines) path ->
    List.fold_left (fun (m, l) seg ->
      match seg with
      | MoveTo _ -> (m + 1, l)
      | LineTo _ -> (m, l + 1)
...
let generate_machine_with_stats ?(fit = true) ?(margin = 10.0) (ir : ir) : string * string =
  let config = machine_config in
  let state = create_state () in
  let scaled_ir = if fit then fit_to_machine ~margin ir else ir in
  if not (check_machine_bounds scaled_ir) then
  ...
  let stats = Printf.sprintf
    ...
    (num_moves + num_lines) num_lines num_moves
    bounds.min_x bounds.max_x bounds.min_y bounds.max_y
    draw_dist post_travel pre_travel
...
let generate_svg ?(width = 800) ?(height = 600) (ir : ir) : string =
  ...
  let path_to_svg path =
    ...
    Buffer.add_string buf (Printf.sprintf "M %.2f %.2f" (tx path.start.x) (ty path.start.y));
    List.iter (fun seg ->
      match seg with
      | MoveTo v -> Buffer.add_string buf (Printf.sprintf " M %.2f %.2f" (tx v.x) (ty v.y))
      | LineTo v -> Buffer.add_string buf (Printf.sprintf " L %.2f %.2f" (tx v.x) (ty v.y))
    ) path.segments;
    Buffer.contents buf
  in
  let paths_svg = ir
    |> List.map (fun path ->
        Printf.sprintf {|  <path d="%s" fill="none" stroke="black" stroke-width="1"/>|} (path_to_svg path))
    |> String.concat "\n"
  in
  Printf.sprintf
