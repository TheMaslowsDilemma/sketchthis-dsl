...
type path_element =
  | Point of point
  | Line of { start: coord; end_: coord }
  | Curve of { start: coord; end_: coord; control_points: coord list }
  | Arc of { start: coord; end_: coord; radius: float }
...
let make_curve start end_ control_points = 
  Curve { start; end_; control_points }
...
let string_of_path_element = function
  ...
  | Line { start; end_ } -> 
      Printf.sprintf "line from %s to %s" 
        (string_of_coord start) (string_of_coord end_)
  | Curve { start; end_; control_points } ->
      let controls = String.concat " and " 
        (List.map string_of_coord control_points) in
      Printf.sprintf "curve from %s to %s through %s"
        (string_of_coord start) (string_of_coord end_) controls
  | Arc { start; end_; radius } ->
      Printf.sprintf "arc from %s to %s with radius %g"
        (string_of_coord start) (string_of_coord end_) radius
